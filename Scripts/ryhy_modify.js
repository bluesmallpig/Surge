...
if($request.url.indexOf("ad/lookVideo")>-1){
   const adheader = JSON.stringify($request.headers)
    if(adheader)
$.setdata(adheader,`ryhyadheader${status}`)
     $.log(`[${zhiyi}] 获取ryhyadheader请求: 成功,adheader: ${adheader}`)
    $.msg(`ryhyadheader${status}: 成功🎉`, ``)
   const adbody = $request.body
   if(adbody)
$.setdata(adbody,`ryhyadbody${status}`)
      $.log(`[${zhiyi}] 获取ryhyadbody请求: 成功,adbody: ${adbody}`)
