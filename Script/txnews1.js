...
const sy = init()
const signurlVal = sy.getdata('sy_signurl_txnews1')
const cookieVal = sy.getdata( 'sy_cookie_txnews1')
const videoVal = sy.getdata( 'video_txnews1')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("article_read")!= -1) {
  ...
  if (signurlVal) sy.setdata(signurlVal, 'sy_signurl_txnews1')
  if (cookieVal) sy.setdata(cookieVal,  'sy_cookie_txnews1')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("video_read")!= -1) {
  ...
  if (videoVal) sy.setdata(videoVal,  'video_txnews1')
...
return new Promise((resolve, reject) => {
  const totalUrl = {
    ...
    if (logs) console.log("获取收益信息" +data)
     const obj = JSON.parse(data)
      subTile = '【微信账号】-【收益总计】'+obj.data.wealth[0].title +'金币  '+"现金: " +obj.data.wealth[1].title+'元'
      }
    resolve()
    })
   })
