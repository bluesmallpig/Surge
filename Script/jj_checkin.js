...
function launch() {
  for (var i in accounts) {
    ...
    if ($.autoLogout) {
      var logouturl = {
        url: "https://j01.best/user/logout",
      };
...
function login(url, email, password, title) {
  let loginPath = `https://j01.best/signin`;
  let table = {
    url:
      `https://j01.best/signin` +
...
function checkin(url, email, password, title) {
  var checkinreqest = "https://j01.best/user/checkin";
...
function dataResults(url, checkinMsg, title) {
  ...
  datarequest = "https://j01.best/user/info";
